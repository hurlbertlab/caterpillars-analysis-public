...
fullDataset = read.csv('data/fullDataset_2026-09-09.csv', header = T)

expert = read.csv('../caterpillars-count-data/2026-08-11_ExpertIdentification.csv')
...
sitePhotos = hopFamiliesBySite %>%
  group_by(Name) %>%
  summarize(sitePhotos = sum(nPhotos)) %>%
  arrange(desc(sitePhotos)) %>%
  filter(sitePhotos >= minNumPhotosPerSite)

hopColors = data.frame(family = c('Flatidae', 'Cicadellidae', 'Membracidae', 'Acanaloniidae', 
                                  'Issidae', 'Derbidae', 'Cicadidae', 'other', 'unknown'),
                        commonName = c('Flatid planthoppers', 'Leafhoppers',
                                       'Treehoppers', 'Acanaloniid planthoppers',
                                       'Issid planthoppers', 'Derbid planthoppers', 'Cicadas', 'other', 'unknown'),
                       color = pie_colors <- c(
                         "steelblue",
                         "tomato",
                         "goldenrod",
