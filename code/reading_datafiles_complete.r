...
users = read.csv(paste('data/', list.files('data')[grep('User.csv', list.files('data'))], sep = ''), header = TRUE, stringsAsFactors = FALSE, fileEncoding="UTF-8-BOM") %>%
  select(ID, FirstName, LastName, Email)

managers = read.csv(paste(github_raw, filter(data_links, grepl("ManagerRequest.csv", file_name))$file_name, sep = ''), header = TRUE, stringsAsFactors = FALSE)
